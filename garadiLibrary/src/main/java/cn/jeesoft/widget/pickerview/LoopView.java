...
public class LoopView extends View {
    ...
    private void e() {
        ...
        for (int i1 = 0; i1 < arrayList.size(); i1++) {
            GaradiItem gItem = arrayList.get(i1);
            String s1 = (String) gItem.getValue();
            ...
            while (j1 < r) {
                ...
                if (f1 >= 90F || f1 <= -90F) {
                    ...
                    if (i2 <= n && h + i2 >= n) {
                        ...
                        canvas.drawText(as[j1].getValue(), startX, h, paintA);
                        canvas.restore();
                        canvas.save();
                        canvas.clipRect(0, n - i2, v, (int) ((float) h * l));
                        canvas.drawText(as[j1].getValue(), startX, h, paintB);
                        canvas.restore();
                    } else if (i2 <= o && h + i2 >= o) {
                        canvas.save();
