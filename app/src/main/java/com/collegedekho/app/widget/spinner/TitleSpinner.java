...
public class TitleSpinner extends LinearLayout {
    ...
    public TitleSpinner(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    public void setFragmentListener(ProfileFragment fragmentListener) {
        TitleSpinner.fragmentListener = fragmentListener;
    }

    private void init(Context context, AttributeSet attrs) {
        this.mContext = context;

        mSpinnerText = new TextView(mContext);
        final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MaterialSpinner);
        int defaultColor = mSpinnerText.getTextColors().getDefaultColor();
        ...
        if (rtl) {
            right = resources.getDimensionPixelSize(R.dimen.m8dp);
        } else {
            left = resources.getDimensionPixelSize(R.dimen.m8dp);
        }
        ...
        setPadding(0, left, 0, right);
        mSpinnerText.setText("Spinner Text");
        mSpinnerText.setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
        mSpinnerText. setPadding(left, top, right, bottom);
        addView(mSpinnerText);
        mSpinnerText.setHeight(resources.getDimensionPixelSize(R.dimen.m40dp));
        mSpinnerText.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mSpinnerText.isEnabled() && mSpinnerText.isClickable()) {
                    if (!popupWindow.isShowing()) {
                        String text = mSpinnerText.getText().toString();
        ...
        mSpinnerText.setBackgroundResource(R.drawable.ic_spinner_selector);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && rtl) {
            mSpinnerText.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
            mSpinnerText.setTextDirection(View.TEXT_DIRECTION_RTL);
        }
        ...
        if (!hideArrow) {
            ...
            if (rtl) {
                mSpinnerText.setCompoundDrawablesWithIntrinsicBounds(arrowDrawable, null, null, null);
            } else {
                mSpinnerText.setCompoundDrawablesWithIntrinsicBounds(null, null, arrowDrawable, null);
            }
        }
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            @Override public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                mSpinnerText.setText(item.getName());
                collapse();
                if (onItemSelectedListener != null) {
                    //noinspection unchecked
                    onItemSelectedListener.onItemSelected(TitleSpinner.this, position, id, item);
                }
            }
        });
        ...
        if (backgroundColor != Color.WHITE) { // default color is white
            mSpinnerText.setBackgroundColor(backgroundColor);
        }
        if (textColor != defaultColor) {
            mSpinnerText.setTextColor(textColor);
        }
        ...
        popupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            @Override public void onDismiss() {
                if (nothingSelected && onNothingSelectedListener != null) {
                    onNothingSelectedListener.onNothingSelected(TitleSpinner.this);
                }
                if (!hideArrow) {
                    animateArrow(false);
                }
    ...
    @Override public void setBackgroundColor(int color) {
        ...
        Drawable background = mSpinnerText.getBackground();
...
/*
    ...
    @Override public void onRestoreInstanceState(Parcelable savedState) {
        if (savedState instanceof Bundle) {
            ...
            if (adapter != null) {
                ...
                mSpinnerText.setText(baseObject.getName());
    ...
    public void setText(String spinnerText){
    ...
    public <T> void setItems(@NonNull List<T> items, boolean isItemSelected) {
        ...
        if(numberOfItems == 1) {
            ...
            mSpinnerText.setCompoundDrawablesWithIntrinsicBounds(null, null, null, null);
        }else{
    ...
    public int getSelectedSpinnerItemId(){
        String  spinnerText = mSpinnerText.getText().toString();
    ...
    private int calculatePopupWindowHeight() {
        ...
        if (popupWindowMaxHeight > 0 && listViewHeight > popupWindowMaxHeight) {
            return (int)getResources().getDimension(R.dimen.m300dp);
    ...
    public interface OnItemSelectedListener<T> {
         ...
         *     The selected item
         */
        void onItemSelected(TitleSpinner view, int position, long id, T item);
    ...
    public interface OnNothingSelectedListener {
         ...
         */
        void onNothingSelected(TitleSpinner spinner);
    }
